...
* e.g. `editeappt 1 str/02/11/19 1800` - allows the user to change a patient's first appointment timing to be the input timing if there is no conflict.
...
`newappt` Command - Adds a appointment or recursive appointments to a patient.
...
`editeappt` Command - Change a current appointment's timing to a different timing.
...
.Interactions Inside the Logic Component when a user executes a `editeappt` command
